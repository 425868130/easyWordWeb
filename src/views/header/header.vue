...
<template>
    <div class="header">
        <!-- 顶部菜单栏 -->
        <Menu mode="horizontal" theme="primary" class="my-menu" @on-select="onMenuClick">
            <router-link to="/wordsMemory">
            ...
            <Input v-model="inputStr" icon="ios-search-strong" placeholder="查词" class="header-search-input" @on-enter="onSearch" @on-click="onSearch"></Input>
            <span v-if="isLogin" class="login-div">
                <Dropdown @on-click="floatMenuClick">
                    <a href="javascript:void(0)">
                        <Avatar icon="person" class="avatar" :src="user.headPortrait" />&nbsp;&nbsp;
                        <span class="userName" v-text="user.nickName"></span>
                        &nbsp;
    ...
    onMenuClick(name) {
      switch (name) {
        case "1":
          ...
          break;
        case "2":
          this.$router.push("/listeningPractice");
          break;
        case "3":
          this.$router.push("/newsPage/1");
          break;
        case "4":
          this.$router.push("/resDownload");
          break;
      }
    },
    floatMenuClick(name) {

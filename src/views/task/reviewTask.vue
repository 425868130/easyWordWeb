...
<template>
  <div class="root-div" v-if="wordsList&&wordsList.length!=0">
    <h1>温故知新&nbsp;<Icon type="ios-book"></Icon></h1>
    <!-- 导航 -->
    <div class="word-nav">
    ...
    <div class="btn-group" v-if="!isReview">
    ...
    <span style="font-size:30px;color:#19be6b" v-if="isReview">
      <Icon type="android-checkmark-circle"></Icon>&nbsp;已复习
    </span>
...
export default {
  ...
  computed: {
    ...
    percent() {
      ...
      if (todayTask != null) {
        return todayTask.reviewProgress * 100 / todayTask.wordNum;
      }
      return 0;
    },
    recentWord() {
    ...
    isReview() {
      /* 获取当前单词是否为已记住状态 */
      if (this.wordsList.length > 0) {
        return this.wordsList[this.index].review;
      } else {
        return false;
      }
    }
  ...
  methods: {
    ...
    rememberWord() {
      ...
      /* 如果任务已完成则不进行操作 */
      if (task.reviewProgress == task.wordNum) {
        return;
      }
      /* 将当前单词的复习标志设为已背诵 */
      this.wordsList[this.index].review = true;
      /* 更新单词列表信息 */
      task.wordList = JSON.stringify(this.wordsList);
      /* 进度加 1 */
      task.reviewProgress++;
      /* 记住当前单词 */
      this.$http.post("/task/updateReviewTask", this.dailyTask).then(res => {
        /* 如果操作成功则更新本地任务信息 */
        if (res.data) {
          this.$store.commit("setReviewTask", task);
          this.nextWord();
          /* 如果当前是最后一个单词，则弹出积分提示 */
          if (task.reviewProgress == task.wordNum) {
            this.$Notice.success({
              title: "任务完成",
              desc: "恭喜你完成复习任务,积分+100"
            });
          }
        }
      });
    ...
    wordLoaded(word) {
      ...
      /* 单词卡片加载完成并且未完成复习则启动定时器 */
      if (word != null && !this.isReview) {
  ...
  created() {
    ...
    if (this.dailyTask == null) {
      this.$store.dispatch("sysReviewTask", false).then(() => {
        this.wordsInit();
      });
    } else {
      this.wordsInit();
